...
//struct for rectangle parameters
#[derive(Copy, Clone, Debug)]
pub struct Rect {
    // x start
    pub x: u32,
...
fn main() {
    let mut imgbuf: image::RgbImage = image::ImageBuffer::new(800, 800); // load buffer
    let shapes: i32 = thread_rng().gen_range(5..15); // randomize amount of shapes
    ...
    imgbuf.save("test1.png").unwrap(); // export png
}

// generate random color for shape
// only red, blue, green, and yellow because of eventual use in a game
...
//generate random circle parameters within range
...
// generate random rectangle parameters within range
...
// make circle struct from circle parameters
...
// make rectangle struct from rectangle parameters
...
// draw rectangle to image buffer
